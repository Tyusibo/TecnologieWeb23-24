<?php
			...
			if(!$ret) {     
				...
				if(pg_num_rows($ret)!=0){  //se ci sono prenotazioni in quella tabella per quell'utente
					...
					while ($prenotazione = pg_fetch_assoc($ret)) {
						echo "<div class=\"card\">";
							echo "<div>Data: " . date("d-m-Y", strtotime($prenotazione["data_appuntamento"])) . "</div>"; //per convertire il formato della data
							echo "<div>Orario: " . substr($prenotazione["orario_appuntamento"], 0, 5) . "</div>"; //per troncare i secondi
	...
	function getPreferenze($id_utente){ 
		...
		if(!$ret) {     
			...
			pg_close($db);
			$numeroPrefenze=4;
			$nessunaPreferenza = true;  //se rimane a true non si ha nessuna preferenze
			for($i=1;$i<$numeroPrefenze;$i+=1)
				if(isset($preferenza["pref_".$i.""]))
					$nessunaPreferenza = false;    
			if($nessunaPreferenza){  //se è rimasto true allora non sono state espresse preferenze
				echo "<p>Non hai preferenze espresse</p>";
				echo "<p><a href=galleria.php class=linkbutton>Qui</a> puoi osservare i vari stili ed esprimerne fino a 3</p>"; 
			}else{
				...
				for($i=1;$i<$numeroPrefenze;$i+=1){
					if(isset($preferenza["pref_".$i.""])){  //se espressa
						echo "<div class=\"flexprefinterno\">";
							...
							echo "<button class=accbutton onclick='cancellaPreferenza(\"pref_".$i."\", $id_utente)'>Cancella</button>";
						echo "</div>";
					}
